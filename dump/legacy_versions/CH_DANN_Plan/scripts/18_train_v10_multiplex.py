...
# ============================================================================
# MODEL: Multiplex GNN-Guided Feature Selection
# ============================================================================
class MultiplexGNNGuided(nn.Module):
    """
    ...
    def forward(self, x, hedge_indices, batch, global_feat=None):
        """
        Args:
        ...
        return self.classifier(mlp_out), attn_weights
...
class MultiplexBatch:
    ...
    def __init__(self, x, y, batch, global_feat, hedge_indices):
        self.x = x
        self.y = y
        self.batch = batch
...
def collate_multiplex(data_list, coexpr_hei):
    ...
    return MultiplexBatch(x, y, batch, global_feat, [kegg_h, str_h, coexpr_h])
...
def train_fold(fold_name, train_data, val_data, n_genes, coexpr_hei, device):
    ...
    model     = MultiplexGNNGuided(n_genes, H_DIM, DROPOUT).to(device)
    ...
    for ep in range(1, EPOCHS + 1):
        ...
        for mbatch in train_loader:
            ...
            optimizer.zero_grad()
            out, attn = model(mbatch.x, mbatch.hedge_indices, mbatch.batch, mbatch.global_feat)
            loss = criterion(out, mbatch.y)
            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            optimizer.step()

            train_loss += loss.item() * mbatch.y.size(0)
        ...
        with torch.no_grad():
            for mbatch in val_loader:
                ...
                out, attn = model(mbatch.x, mbatch.hedge_indices, mbatch.batch, mbatch.global_feat)
                loss = criterion(out, mbatch.y)
                val_loss_total += loss.item() * mbatch.y.size(0)
...
def main():
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(sgkf.split(range(len(data_list)), labels, batches)):
        ...
        if model is None:
            all_results.append({
                ...
                'acc': float('nan'), 'auroc': float('nan'), 'f1': float('nan'),
        ...
        fold_path = os.path.join(MODEL_DIR, f"v10_multiplex_fold{fold_idx+1}.pt")
        ...
        all_results.append({
            'fold': fold_idx + 1, 'held_out': ', '.join(held_out),
            'acc': metrics['acc'], 'auroc': metrics['auroc'], 'f1': metrics['f1'],
    ...
    if best_overall_state:
        best_path = os.path.join(MODEL_DIR, "v10_multiplex_best.pt")
    ...
    # ══════════════════════════════════════════
    print(f"\n{'='*80}")
    print("  FINAL RESULTS: V10 Multiplex (KEGG+STRING+CoExpr) — Ranked by Accuracy")
    print(f"{'='*80}")
    print(f"  {'Fold':>4s} | {'Held-Out':<16s} | {'N':>4s} | {'Acc':>7s} | {'AUC':>7s} | {'F1':>6s} | {'αK':>5s} {'αS':>5s} {'αC':>5s}")
    print(f"  {'-'*4}-+-{'-'*16}-+-{'-'*4}-+-{'-'*7}-+-{'-'*7}-+-{'-'*6}-+-{'-'*5}-{'-'*5}-{'-'*5}")

    valid_accs, valid_aurocs, valid_f1s = [], [], []
    for r in all_results:
        if r['skipped']:
            print(f"  {r['fold']:4d} | {r['held_out']:<16s} | {r['n_val']:4d} | {'SKIP':>7s} |")
        else:
            print(f"  {r['fold']:4d} | {r['held_out']:<16s} | {r['n_val']:4d} | {r['acc']:7.4f} | {r['auroc']:7.4f} | {r['f1']:6.3f} | "
    ...
    if valid_accs:
        print(f"  {'-'*4}-+-{'-'*16}-+-{'-'*4}-+-{'-'*7}-+-{'-'*7}-+-{'-'*6}-+-{'-'*5}-{'-'*5}-{'-'*5}")
        print(f"  {'Mean':>4s} | {'':16s} | {'':4s} | {np.mean(valid_accs):7.4f} | {np.mean(valid_aurocs):7.4f} | {np.mean(valid_f1s):6.3f} |")
        print(f"  {'Std':>4s}  | {'':16s} | {'':4s} | {np.std(valid_accs):7.4f} | {np.std(valid_aurocs):7.4f} | {np.std(valid_f1s):6.3f} |")

    # 4-way comparison
    print(f"\n  ══ 4-Way Comparison (Accuracy) ══")
    comparisons = []
    ...
    comparisons.append(("V10 Multiplex", np.mean(valid_accs), np.mean(valid_aurocs), gse_v10))
    ...
    summary = {
        'method': 'Multiplex HypergraphConv + GNN-Guided (V10)',
        'architecture': '3 relations (KEGG+STRING+CoExpr) → attention → gene scoring → MLP',
    ...
    json_path = os.path.join(OUT_DIR, "v10_multiplex_results.json")
