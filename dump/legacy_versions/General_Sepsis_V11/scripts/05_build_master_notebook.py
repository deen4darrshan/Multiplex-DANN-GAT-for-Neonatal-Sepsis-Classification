...
def write_tex(root: Path, out_tex: Path):
    ...
    lines.append(r"\section{Critical File Walkthrough}")
    walkthrough = [
        (
            "General_Sepsis_V11/scripts/01_download_and_preprocess.py",
            "Locks cohort policy, executes deterministic fallback handling, runs train-only ComBat harmonization, selects MAD-ranked genes, and exports the canonical expression/metadata artifacts used everywhere else in the branch.",
        ),
        (
            "General_Sepsis_V11/scripts/02_build_graphs.py",
            "Builds KEGG and STRING priors for the selected genes and documents that co-expression is rebuilt per fold instead of being persisted globally, which is essential for leakage control.",
        ),
        (
            "General_Sepsis_V11/scripts/03_train_v11_general_sepsis.py",
            "Implements the multiplex hypergraph plus DANN training loop, including fold-internal feature selection, fold-local normalization, co-expression rebuilding, checkpointing, and relation-attention trace capture.",
        ),
        (
            "General_Sepsis_V11/scripts/04_evaluate.py",
            "Scores the hybrid model, rebuilds matched baselines on identical folds, performs permutation testing, runs external holdout inference, and writes the audit-facing JSON and markdown outputs.",
        ),
        (
            "General_Sepsis_V11/scripts/06_metrics_and_plots.py",
            "Turns raw result payloads into ROC/PR overlays, heatmaps, topology renders, SHAP summaries, and the reviewer-readable metrics report consumed by poster/publication packaging.",
        ),
        (
            "General_Sepsis_V11/results/cv_metrics_raw.json",
            "Stores the fold definitions, selected genes, normalization statistics, epoch traces, OOF predictions, and checkpoint references needed to audit the full training run.",
        ),
        (
            "General_Sepsis_V11/results/general_sepsis_v11_results.json",
            "Acts as the top-level results contract for the rebuilt sepsis architecture by aggregating CV metrics, external performance, confidence intervals, baseline summaries, and hard-pass gates.",
        ),
        (
            "CH_DANN_Plan/scripts/19_train_v11_multiplex_dann.py",
            "Captures the architectural lineage immediately preceding the robust rebuild and explains where the current multiplex DANN design choices originated.",
        ),
        (
            "CH_DANN_Plan/scripts/22_train_v11_alzheimers_transfer.py",
            "Shows that the graph-guided and domain-adversarial design was extended into a second disease area, supporting broader platform claims.",
        ),
        (
            "Osteogenesis imperfecta/scripts/04_baselines.py",
            "Provides the rare-disease tabular baseline reference used to judge whether graph-based methods still add value when sample counts are small and cohorts are heterogeneous.",
        ),
        (
            "Osteogenesis imperfecta/scripts/05_train_gnn.py",
            "Implements the rare-disease GNN path and therefore anchors the notebook's cross-disease generalization argument.",
        ),
        (
            "ACSEF_Final_Submission/scripts/02_compile_metrics_and_make_figures.py",
            "Collects metrics across branches, standardizes naming, and generates the ACSEF-facing visual and narrative assets used in the final submission package.",
        ),
        (
            "ACSEF_Final_Submission/scripts/05_generate_general_sepsis_v11_publication.py",
            "Assembles the regenerated sepsis figures into poster exports, figure manifests, and claim-traceability tables so the final package can be reproduced from source.",
        ),
    ]
    lines.append(r"\begin{itemize}")
    for rel, desc in walkthrough:
        lines.append(r"\item \texttt{" + esc(rel) + r"}: " + esc(desc))
    lines.append(r"\end{itemize}")
