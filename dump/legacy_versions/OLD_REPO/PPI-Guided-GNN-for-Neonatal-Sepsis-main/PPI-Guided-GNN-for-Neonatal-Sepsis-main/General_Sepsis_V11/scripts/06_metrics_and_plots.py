...
def main() -> None:
    ...
    if train_ids and hold_ids:
        ...
        models = {
            "logistic_regression_refit": LogisticRegression(max_iter=3000, class_weight="balanced", random_state=args.seed),
            "mlp_only_refit": MLPClassifier(hidden_layer_sizes=(128,), activation="relu", alpha=1e-4, max_iter=700, random_state=args.seed),
            "v12_no_mlp_ablation_refit": SGDClassifier(loss="log_loss", penalty="l2", alpha=1e-4, max_iter=5000, tol=1e-4, random_state=args.seed),
        }
        for name, model in models.items():
            m = model
            m.fit(X_train_s, y_train)
            if hasattr(m, "predict_proba"):
                p_train = m.predict_proba(X_train_s)[:, 1]
                p_hold = m.predict_proba(X_hold_s)[:, 1]
            else:
                d_train = m.decision_function(X_train_s).astype(np.float64)
                d_hold = m.decision_function(X_hold_s).astype(np.float64)
