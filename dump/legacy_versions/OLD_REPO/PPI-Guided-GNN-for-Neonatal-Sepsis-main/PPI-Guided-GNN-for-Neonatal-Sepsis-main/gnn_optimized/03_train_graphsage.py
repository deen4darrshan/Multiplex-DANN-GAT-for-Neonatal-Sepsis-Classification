"""
GNN Optimized - Step 3: Train GraphSAGE

Train GraphSAGE with same hyperparameters as optimized GCN.
...
print("=" * 70)
print("GNN OPTIMIZED - GraphSAGE TRAINING")
print("=" * 70)
...
class OptimizedGraphSAGE(nn.Module):
    """3-Layer GraphSAGE with optimized architecture."""
...
def train_fold(fold, train_data, val_data, in_channels):
    ...
    model = OptimizedGraphSAGE(in_channels, HIDDEN_CHANNELS, num_classes=2, dropout=DROPOUT).to(device)
...
def main():
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(range(len(data_list)), labels)):
        ...
        # Save fold model
        torch.save(model_state, os.path.join(MODEL_DIR, f"graphsage_fold{fold+1}.pt"))
    ...
    }, os.path.join(MODEL_DIR, "graphsage_best.pt"))
    ...
    results_md = f"""# GraphSAGE Optimized Results
...
| Model | Mean AUROC | Best Fold |
|-------|------------|-----------|
| GraphSAGE | {mean_auroc:.4f} | {best_overall_auroc:.4f} |
| GCN Optimized | 0.6851 | 0.8107 |
| LR Baseline | 0.8164 | - |
"""
    
    with open(os.path.join(LOG_DIR, "graphsage_results.md"), 'w') as f:
        f.write(results_md)
    
    print(f"\nResults saved to: {LOG_DIR}/graphsage_results.md")
