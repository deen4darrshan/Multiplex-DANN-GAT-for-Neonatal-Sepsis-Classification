    ...
    for name, model in baseline_defs.items():
        ...
        for fold in folds:
            ...
            if name == "logistic_regression":
                X_tr = build_logistic_features(X_tr)
                X_va = build_logistic_features(X_va)
            elif name == "mlp_only":
                X_tr = build_mlp_features(X_tr)
                X_va = build_mlp_features(X_va)
            elif name == "v12_no_mlp_ablation":
                X_tr = build_linear_ablation_features(X_tr)
                X_va = build_linear_ablation_features(X_va)
        ...
        out[name] = {
            ...
            "config": (
                {
                    "feature_policy": "summary_plus_bins_plus_tail",
                    "summary_features": ["mean", "std", "median", "iqr"],
                    "bin_count": STRICT_LOGREG_BIN_COUNT,
                    "tail_features": STRICT_LOGREG_TAIL_FEATURES,
                    "C": 0.02,
                    "penalty": "l2",
                    "solver": "liblinear",
                    "class_weight": None,
                ...
                if name == "logistic_regression"
                else (
                    {
                        "feature_policy": "summary_plus_bins_plus_tail",
                        "summary_features": ["mean", "std", "median", "iqr"],
                        "bin_count": STRICT_MLP_BIN_COUNT,
                        "tail_features": STRICT_MLP_TAIL_FEATURES,
                    }
                    if name == "mlp_only"
                    else (
                        {
                            "feature_policy": "summary_plus_bins_plus_tail",
                            "summary_features": ["mean", "std", "median", "iqr"],
                            "bin_count": STRICT_LINEAR_BIN_COUNT,
                            "tail_features": STRICT_LINEAR_TAIL_FEATURES,
                        }
                        if name == "v12_no_mlp_ablation"
                        else {}
                    )
                )
            ),
